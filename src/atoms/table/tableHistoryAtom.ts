...
export const canUndoTableAtom = atom((get) => {
  ...
  // Check if there's meaningful history (not just the initial empty state)
  if (history.undoStack.length === 0) return false;
  
  // Check if the last item has actual changes (not just empty arrays)
  const lastSnapshot = history.undoStack[history.undoStack.length - 1];
  return (
    lastSnapshot.sorting.length > 0 || 
    lastSnapshot.filters.length > 0 || 
    lastSnapshot.columnOrder.length > 0
  );
});
