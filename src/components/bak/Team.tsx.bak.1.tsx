...
const TeamMemberHeader = ({ member, index, onEdit, isEditing, editValue, onEditChange, onSave, onCancel }) => {
  return (
    <th className="w-32 h-12 text-center sticky top-0 z-10 bg-gray-600 text-white font-semibold text-sm">
      {isEditing ? (
        ...
        <div className="flex items-center justify-center gap-1">
          <button
            onClick={onEdit}
            className="text-sm font-semibold hover:bg-white/20 rounded transition-colors px-1 text-center"
      ...
      <table className="w-full">
        <thead>
          <tr>
            ...
            <th className="w-24 h-12 bg-orange-500 text-white font-semibold sticky top-0 z-10 border-none text-sm">
              <button 
                onClick={onAddTeamMember}
                className="w-full h-full bg-transparent border-none text-white hover:bg-orange-600 transition-colors rounded flex items-center justify-center gap-1 text-sm font-semibold"
