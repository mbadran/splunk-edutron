...
const Catalog = forwardRef<CatalogRef, CatalogProps>(({ onScroll }, ref) => {
  // Debug logging with circuit breaker
  const debugInfo = useRenderDebugger("Catalog", {
    onScrollDefined: !!onScroll,
  });

  // CIRCUIT BREAKER: Stop rendering if excessive renders detected
  if (debugInfo.isCritical) {
    return (
      <div className="h-full flex items-center justify-center bg-red-100 border-2 border-red-500">
        <div className="text-center p-8">
          <h2 className="text-2xl font-bold text-red-800 mb-4">
            Component Stopped - Infinite Render Detected
          </h2>
          <p className="text-red-600 mb-4">
            Catalog has been stopped due to excessive renders (
  ...
  const columns: ColumnDef<Course>[] = React.useMemo(() => {
    ...
    // Add row number column first (far left) - disable sorting since it's just a counter
    allColumns.push(
      columnHelper.display({
        id: "#",
        header: () => (
          <div className="flex items-center gap-2 w-full">
            <div className="flex items-center gap-1 px-1 py-1 rounded transition-colors flex-1 min-w-0">
        ...
        enableResizing: true,
        enableSorting: false, // Disable sorting for row numbers
      }),
    );
