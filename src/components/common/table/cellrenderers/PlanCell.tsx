...
// FIXED: Plan cell with only selection squares (no background colors) + CSS classes for testing
export const PlanCell = React.memo<PlanCellProps>(
  ({ course, member, isSelected, onToggle }) => {
    return (
      <div 
        className="h-12 flex items-center justify-center px-2 cursor-pointer transition-all duration-200 group hover:bg-orange-50"
      ...
      >
        <div
          className={`w-6 h-6 flex items-center justify-center rounded transition-all duration-200 ${
            isSelected
              ? "bg-emerald-400 plan-cell-selected group-hover:border-2 group-hover:border-orange-400"
              : "border-2 border-gray-300 plan-cell-unselected group-hover:border-orange-400"
          }`}
        >
          {/* No inner content needed */}
        </div>
      </div>
    );
  },
