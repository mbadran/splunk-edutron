import React from "react";
import { useAtom } from "jotai";
import Link from "next/link";
import {
  Upload,
  Plus,
  Users,
  Calculator,
  ChevronRight,
  GraduationCap,
  BookOpen,
} from "lucide-react";
import { setStatusAtom } from "@/atoms/globalAtoms";

const HomePage: React.FC = () => {
  const [, setStatus] = useAtom(setStatusAtom);

  const handleBrowseCatalogs = () => {
    setStatus({ isWorking: true, message: "Navigating to Catalogs..." });
    setTimeout(() => {
      setStatus({ isWorking: false, message: "" });
    }, 100);
  };

  const handleCreatePlan = () => {
    setStatus({ isWorking: true, message: "Opening Planner..." });
    setTimeout(() => {
      setStatus({ isWorking: false, message: "" });
    }, 100);
  };

  const handleLoadPlan = () => {
    setStatus({ isWorking: true, message: "Loading Plan Upload..." });
    setTimeout(() => {
      setStatus({ isWorking: false, message: "" });
    }, 100);
  };

  return (
    <div
      id="homepage-container"
      className="min-h-screen flex items-center justify-center p-4 relative"
      style={{
        backgroundImage: "url(/img/luis-tosta-COAbFWdOf5s-unsplash.jpg)",
        backgroundSize: "cover",
        backgroundPosition: "center",
        backgroundRepeat: "no-repeat",
      }}
    >
      <div className="max-w-2xl mx-auto text-center">
        <div
          className="backdrop-blur-md rounded-3xl p-12 shadow-2xl border border-white/20"
          style={{
            background:
              "linear-gradient(135deg, #475569 0%, #334155 30%, #1e40af 70%, #0c1724 100%)",
          }}
        >
          <div className="mb-8">
            <div className="inline-flex items-center justify-center w-20 h-20 bg-white/20 backdrop-blur-sm rounded-2xl mb-6">
              <GraduationCap className="w-10 h-10 text-white" />
            </div>
            <h1 className="text-5xl md:text-6xl font-bold text-white mb-4 tracking-tight">
              Splunk{" "}
              <span className="bg-gradient-to-r from-orange-500 to-pink-500 bg-clip-text text-transparent">
                EDUTRON
              </span>
            </h1>
            <p className="text-lg text-white/90 font-light mb-2">
              <em>The Missing Splunk Training Planner</em>
            </p>
          </div>

          <div className="space-y-4 mb-8 max-w-sm mx-auto">
            <Link
              href="/catalogs"
              onClick={handleBrowseCatalogs}
              className="w-full bg-gradient-to-r from-orange-500 to-pink-500 text-white hover:from-orange-600 hover:to-pink-600 font-bold py-4 px-8 rounded-xl transition-all duration-200 transform hover:scale-105 hover:shadow-lg flex items-center justify-center gap-3 drop-shadow-sm"
              style={{ textShadow: "0 1px 2px rgba(0, 0, 0, 0.3)" }}
              aria-label="Browse all training catalogs"
            >
              <BookOpen className="w-5 h-5" />
              Browse All Catalogs
            </Link>

            <Link
              href="/planner"
              onClick={handleCreatePlan}
              className="w-full bg-gradient-to-r from-orange-500 to-pink-500 text-white hover:from-orange-600 hover:to-pink-600 font-bold py-4 px-8 rounded-xl transition-all duration-200 transform hover:scale-105 hover:shadow-lg flex items-center justify-center gap-3 drop-shadow-sm"
              style={{ textShadow: "0 1px 2px rgba(0, 0, 0, 0.3)" }}
              aria-label="Create a new training plan"
            >
              <Plus className="w-5 h-5" />
              Create Training Plan
            </Link>

            <Link
              href="/planner?action=upload"
              onClick={handleLoadPlan}
              className="w-full bg-gradient-to-r from-orange-500 to-pink-500 text-white hover:from-orange-600 hover:to-pink-600 font-bold py-4 px-8 rounded-xl transition-all duration-200 transform hover:scale-105 hover:shadow-lg flex items-center justify-center gap-3 drop-shadow-sm"
              style={{ textShadow: "0 1px 2px rgba(0, 0, 0, 0.3)" }}
              aria-label="Upload an existing training plan"
            >
              <Upload className="w-5 h-5" />
              Upload Existing Plan
            </Link>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-white/80">
            <div id="step-integration" className="flex items-center gap-2">
              <ChevronRight className="w-4 h-4" />
              <span>STEP Integration</span>
            </div>
            <div id="interactive-planning" className="flex items-center gap-2">
              <Calculator className="w-4 h-4" />
              <span>Budget Calculator</span>
            </div>
            <div id="team-support" className="flex items-center gap-2">
              <Users className="w-4 h-4" />
              <span>Team Support</span>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default HomePage;
